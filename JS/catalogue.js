...
function applyFilters() {
    ...
    if (searchResults) searchResults.textContent = totalVisible === 1 ? "1 carte trouvée en utilisant les filtres de recherche" : `${totalVisible} cartes trouvées en utilisant les filtres de recherche`;
}
applyFilters();
...
(function(){
    ...
    function expansionCaptionFallback(expansion){
        ...
        if (expansionNames[expansion]) return 'Disponible dans l\'extension ' + expansionNames[expansion];
        const formattedExpansion = expansion.replace(/_/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
        return 'Disponible dans l\'extension ' + formattedExpansion;
    }
